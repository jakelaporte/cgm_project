...
class CGM(object):
    def __init__(self,filename,dt_col=1,gl_col=7):
        ...
        self.series = self.read_data(filename,dt_col=dt_col,gl_col=gl_col)
        ...
        self.set_params({'type':'paper','unit':'mg','lower':70,'conga_h':1,
                         ...
                         'above':True,'m_index':120,'li_k':60,
                         'data':self.return_period_data(True),
                         'day_data':self.return_time_data('6:00','23:55'),
                         'night_data':self.return_time_data('00:00','06:00')})
        self.N = len(self.params['data'])
    ...
    def read_data(self,filename,dt_fmt='%Y-%m-%d %H:%M',dt_col=1,gl_col=2):
        def adjust_times(x):
            dt,tm =x.split(' ') 
            tm = tm.split(':')
            tm[0]=int(tm[0])
        ...
        self.periods = self.build_periods(data.copy())
        series = pd.DataFrame()
        #get rid of duplicated datetimes
        data = data[~data.index.duplicated()]
        for period in self.periods:
            temp = pd.DataFrame([],index = self.generate_range(period[0],period[1]))
    ...
    def build_daily_dataframe(self):
        ...
        self.days = [day.strftime("%m/%d/%Y") for day in 
                     pd.to_datetime(series['day'].unique())]
        total_time = timedelta(0)
        total_seconds=total_minutes=total_hours=total_days=0
        for period in self.periods:
            ...
            series.loc[period[0]:period[1],'imputed'] = self.linear_interpolate(ser['glucose'].values,indices)
    ...
    def build_periods(self,series):
        ...
        idxs = list(series[series['time_diff']>288][['dates','date_shift']].values)
        periods = [[ts[0],ts[-1]]]
        for idx in idxs:
    ...
    def time_in_range(self,**kwargs):
        ...
        below_range = len(data[data<lower])
        in_range = len(data[(data>=lower) & (data<=upper)])
        above_range = len(data[data>upper])
    ...
    def average_daily_risk_range(self,**kwargs):
        ...
        if len(days)>=1:
            ...
            for i,day in enumerate(days):
                rh_data = self.return_data(df,'rh',day)
                rl_data = self.return_data(df,'rl',day)
                LR[i]=max(rl_data)
                HR[i]=max(rh_data)
            adrr_m = (LR+HR).mean()
            adrr_l = LR.mean()
    ...
    def assign_functions(self):
        ...
        self.stats_functions['cv']['f'] = self.glucose_cv
        ...
        self.stats_functions['j_index']['f']=self.j_index
        ...
        self.stats_functions['bgi']['f']=self.low_high_blood_glucose_index
        ...
        self.stats_functions['grade']['f']=self.glycemic_risk_assessment_diabetes_equation
        self.stats_functions['grade']['description']='GRADE'
        self.stats_functions['grade']['normal']=[0,4.7] #Hill 2011
        
        self.stats_functions['gvp']={}
        self.stats_functions['gvp']['f']=self.glycemic_variability_percentage
        ...
        self.stats_functions['li']['f']=self.lability_index
        ...
        self.stats_functions['m_value']['f'] = self.m_value
        ...
        self.stats_functions['gmi']['f'] = self.glucose_management_indicator
        self.stats_functions['gmi']['description']='eA1C'
        self.stats_functions['gmi']['normal']=[0,6] #Danne 2017
        
        self.stats_functions['iqr']={}
        self.stats_functions['iqr']['f'] = self.interquartile_range
    ...
    def calculate_stats(self):
        ...
        self.stats['percent_active']=self.percent_active()
        self.stats['mean']=self.glucose_mean(**inp)
        self.stats['std']=self.glucose_std(**inp)
        self.stats['num_obs']=self.glucose_N(**inp)
        self.stats['cv'] = self.glucose_cv(**inp)
        print("1")
        self.stats['mag']=self.mean_absolute_glucose(**inp)
        self.stats['tir']=self.time_in_range(**inp)
        self.stats['j_index'] = self.j_index(**inp)
        self.stats['glycemic_variability_percentage']=self.glycemic_variability_percentage(**inp)
        self.stats['lability_index']=self.lability_index(**inp)
        print("2")
        self.stats['GRADE_'+inp['type']] = self.glycemic_risk_assessment_diabetes_equation(**inp)
        self.stats['MODD_'+inp['type']]=self.mean_of_daily_differences(**inp)
        self.stats['ADRR_'+inp['type']]=self.average_daily_risk_range(**inp)       
        self.stats['blood_glucose_index_'+inp['type']]=self.low_high_blood_glucose_index(**inp)
        print("3")
        self.stats['conga1_'+inp['type']]=self.conga(**inp)
        self.stats['M_value_'+inp['type']]=self.m_value(**inp)
        self.stats['auc_100+']=self.auc(**inp)
        inp['above']=False
        self.stats['auc_100-']=self.auc(**inp)
        inp['above']=True
        self.stats['eA1C']=self.glucose_management_indicator(**inp)
        print("4")
        self.stats['irq']=self.interquartile_range(**inp)
        self.stats['wake_AUC']=self.auc_5(day='day')
        self.stats['sleep_AUC']=self.auc_5(day='night')
        self.stats['24hour_AUC']=self.auc_5(day='all')
        self.stats['MAGE']=self.mean_amplitude_of_glycemic_excursions(**inp)
        self.stats['gri']=self.glycemic_risk_index(**inp)
        ...
        self.stats['MODD_'+inp['type']]=self.mean_of_daily_differences(**inp)
        self.stats['M_value_'+inp['type']]=self.m_value(**inp)
        self.stats['ADRR_'+inp['type']]=self.adrr_easy(**inp)
        inp['type']='paper'
    ...
    def plot_agp(self):
        ...
        ## % Time CGM Active
        vals = [self.stats['percent_time_CGM_active']*100]
        norms = [""]
    ...
    def plot_gri(self):
        ...
        """
        points = self.glycemic_risk_index(**self.params)
